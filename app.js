...
const elements = {
  ...
  btnTempRegister:          document.getElementById('btn-temp-register'),
...
function getFirebaseErrorMessage(code) {
  const msgs = {
    ...
    'auth/unauthorized-domain':      'โดเมนนี้ยังไม่ได้รับอนุญาตใน Firebase (กรุณาเพิ่มโดเมนใน Firebase Console)'
  };
...
async function handleRegisterSubmit() {
  ...
  if (!email || !password) {
    showLoginError('กรุณากรอกอีเมลและรหัสผ่านเพื่อสมัครสมาชิก');
    return;
  }
  if (password.length < 6) {
    showLoginError('รหัสผ่านต้องมีความยาวอย่างน้อย 6 ตัวอักษร');
    return;
  }

  showLoginError('');
  const btn = elements.btnTempRegister;
  const oldText = btn.innerHTML;
  btn.disabled    = true;
  btn.innerHTML   = '<i class="fa-solid fa-circle-notch fa-spin"></i> กำลังสมัคร...';
  ...
  try {
    ...
    await db.collection('users').doc(cred.user.uid).set({
      email: email,
      displayName: 'ผู้ดูแลระบบ (ลงทะเบียนใหม่)',
      role: 'admin',
      disabled: false,
      createdAt: firebase.firestore.FieldValue.serverTimestamp()
    });
    alert('สมัครสมาชิกสำเร็จแล้ว! ระบบจะนำคุณเข้าสู่ระบบโดยอัตโนมัติ');
    elements.formLogin.reset();
  } catch (err) {
    showLoginError(getFirebaseErrorMessage(err.code));
  } finally {
...
function switchTab(tabName) {
  ...
  const tabInfo = {
    ...
    statement:     { title: 'บัญชีเดินสะพัด', subtitle: 'สรุปรายการเงินเข้าและเงินออก' }
  };
  ...
  if (tabName === 'statement')     renderStatementTab();
}
...
function renderPagination() {
  ...
  const cont = elements.paginationContainer;
  if (!cont) return;
  
  let html = `<button class="btn btn-sm btn-outline" ${currentPage === 1 ? 'disabled' : ''} onclick="AppState.pagination.currentPage--; renderDisbursementsTable()">ก่อนหน้า</button>`;
  html += `<span style="margin: 0 10px;">หน้า ${currentPage} จาก ${totalPages}</span>`;
  html += `<button class="btn btn-sm btn-outline" ${currentPage === totalPages ? 'disabled' : ''} onclick="AppState.pagination.currentPage++; renderDisbursementsTable()">ถัดไป</button>`;
  cont.innerHTML = html;
}
...
function populateShopFilterDropdown() {
  ...
  elements.filterShop.innerHTML = '<option value="">-- ทุกร้านค้า --</option>' + 
...
function setupEventListeners() {
  ...
  elements.btnTempRegister?.addEventListener('click', handleRegisterSubmit);
...
function renderStatementTab() {
  const shopSel = document.getElementById('st-shop');
  ...
  if (!window.statementListenersAttached) {
    ['st-shop','st-type','st-start','st-end'].forEach(id => {
      document.getElementById(id)?.addEventListener('change', renderStatementTable);
    });
    document.getElementById('btn-clear-st-filters')?.addEventListener('click', () => {
      ['st-shop','st-type','st-start','st-end'].forEach(id => document.getElementById(id).value = '');
      renderStatementTable();
    });
    document.getElementById('btn-export-st-csv')?.addEventListener('click', () => {
      const filtered = getFilteredStatement();
      if(filtered.length === 0) return showToast('ไม่พบข้อมูลสำหรับส่งออก', 'warning');
      const csvContent = "data:text/csv;charset=utf-8,\uFEFF" 
        + "วันที่,เลขที่เอกสาร,ร้านค้า,รายละเอียด,ยอดก่อนภาษี,VAT,หัก ณ ที่จ่าย,เงินเข้า (+),เงินออก (-),ช่องทาง,สถานะ,ประเภท\n"
        + filtered.map(r => {
            const shopObj = AppState.shops.find(s => s.id === r.shopId);
            const shopName = shopObj ? shopObj.name : 'N/A';
            const vIn = isMoneyIn(r) ? r.netAmount : 0;
            const vOut = !isMoneyIn(r) ? r.netAmount : 0;
            const t = isMoneyIn(r) ? 'เงินเข้า' : 'เงินออก';
            return `"${r.date}","${r.ref}","${shopName}","${(r.details||'').replace(/"/g,'""')}",${r.subtotal},${r.vatAmount||0},${r.whtAmount||0},${vIn},${vOut},"${r.paymentMethod}","${r.status}","${t}"`;
          }).join("\n");
    ...
    window.statementListenersAttached = true;
  }
  
  renderStatementTable();
}

function getFilteredStatement() {
  const shopF = document.getElementById('st-shop')?.value;
  const typeF = document.getElementById('st-type')?.value;
  const startF = document.getElementById('st-start')?.value;
  const endF = document.getElementById('st-end')?.value;
  
  return AppState.disbursements.filter(r => {
    if (r.status === 'Cancelled') return false; 
    if (shopF && r.shopId !== shopF) return false;
    if (startF && r.date < startF) return false;
    if (endF && r.date > endF) return false;
    if (typeF) {
      const isMIn = isMoneyIn(r);
      if (typeF === 'in' && !isMIn) return false;
      if (typeF === 'out' && isMIn) return false;
    }
    return true;
  }).sort((a,b) => new Date(b.date) - new Date(a.date));
}
...
function renderStatementTable() {
  ...
  const filtered = getFilteredStatement();
  document.getElementById('st-records-count').textContent = `${filtered.length} รายการ`;
  
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="12" class="text-center text-muted py-4">ไม่พบข้อมูล</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = filtered.map(r => {
    ...
    const shopName = shopObj ? shopObj.name : 'N/A';
    const isMIn = isMoneyIn(r);
    const inAmt = isMIn ? formatCurrency(r.netAmount) : '-';
    const outAmt = !isMIn ? formatCurrency(r.netAmount) : '-';
    ...
    return `<tr>
      ...
      <td class="text-right">${r.vatType === 'none' ? '-' : formatCurrency(r.vatAmount)}</td>
      <td class="text-right">${r.whtRate > 0 ? `(${formatCurrency(r.whtAmount)})` : '-'}</td>
      <td class="text-right font-weight-bold" style="color:#10b981">${inAmt}</td>
      <td class="text-right font-weight-bold" style="color:#ef4444">${outAmt}</td>
      <td>${r.paymentMethod}</td>
      <td>${getStatusBadge(r.status)}</td>
      <td class="text-center">${getTransactionBadge(r)}</td>
    </tr>`;
